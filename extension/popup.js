...
async function searchJobs() {
  ...
  console.log(`Initiating search request for keyword='${keyword}' location='${location}'`);
  showMessage("Searching jobs...", "info");
  ...
  try {
    ...
    showMessage(
      `${data.jobs?.length || 0} jobs found. ${sourceLabel}.`,
      "success"
    );

    if (data.backgroundScrape) {
      showMessage(
        `${data.jobs?.length || 0} cached jobs shown. Refreshing live results...`,
        "success"
      );

      setTimeout(() => {
        refreshSearch(keyword, location);
      }, 5000);
    }
...
async function refreshSearch(keyword, location) {
  try {
    ...
    if (!data.success) {
      throw new Error(data.message || "Job search failed.");
    }

    if (data.jobs?.length > 0) {
      displayJobs(data.jobs);

      const sourceLabel = data.source === "cache" ? "Cached results" : "Live results";
      showMessage(`${data.jobs.length} jobs found. ${sourceLabel}.`, "success");
    } else {
      showMessage(
    ...
    showMessage("Live refresh failed. Showing cached results.", "error");
  }
}
...
function displayJobs(jobs) {
  if (!resultsSection || !jobsList || !resultCount) {
    console.error(
